...
;(async () => {
  try {
    ...
    // Phase 1.5: Family team swaps (keep minors with parents)
    console.log('\nPhase 1.5: Family team reassignment...')
    ;(function() {
      function setTeam(wcif, wcaId, userId, team) {
        const p = wcaId ? wcif.persons.find(pp => pp.wcaId === wcaId) : wcif.persons.find(pp => pp.wcaUserId === userId)
        ...
        const oldTeam = ext.data.properties['staff-team']
        ext.data.properties['staff-team'] = team
        console.log('  ' + p.name.substring(0,30) + ': T' + (oldTeam||'?') + ' → T' + team)
      }
      // Familia Giraldo-Quintero → all to T2 (where Mariana is)
      setTeam(afterPhase1, '2022QUIN17', null, 2)  // Samuel T3→T2
      setTeam(afterPhase1, null, 474236, 2)         // Ana Milena T1→T2
      // Swaps to maintain balance
      setTeam(afterPhase1, null, 440824, 1)         // Johana Suarez T2→T1
    ...
    // Phase 3.5: Assign unofficial event competitors to groups
    console.log('\nPhase 3.5: Assigning unofficial competitors to groups...')
    ;(function assignUnofficialCompetitors() {
      const actMap = {}
      ...
      function hasConflict(person, startUTC, endUTC) {
        for (const a of person.assignments || []) {
          const info = actMap[a.activityId]
          if (!info) continue
          if (info.start < endUTC && startUTC < info.end) return true
        }
        return false
      }
      ...
      const events = [
        {
          name: 'Mirror', groups: [121, 469, 470],
          competitors: [
            '2018KUMA01','2023RODR80','2023BELT07','2023SUAR08','2022QUIN17','2021SOTO01',
            '2019GUTI14','2012MARI04','2016CRUZ16','2016BARO02','2024GUTI02','2024HENA04',
            '2023CELI06','2024COLO04','2023MORR23','2023MORE31','2025FUEN05','2023FLOR03',
            '2021OROZ01','2022CUER01','2016CHAV12','2015TERR01','2015TORR12','2017MART94',
            '2023VELE05','2025COND15','2012PERE04','2019DEVE02','2023LOPE96','2024LOPE19',
            '2023PINH03','2024RINC06','2019RODR66','2014BULU01','2015OSPI01','2023STRA33',
            '2023GODO07','2023VALD02','2023VALD03','2023MAZU05','2024ORTI07','2018GONZ30',
            '2023GOME49','2016SUZU03'
          ]
        },
        {
          name: 'Kilominx', groups: [122, 471, 472],
          competitors: [
            '2023RODR80','2023SUAR08','2018KUMA01','2021SOTO01','2022MUNA04','2012MARI04',
            '2016CRUZ16','2016BARO02','2019BUIT01','2023CELI06','2024COLO04','2023MORR23',
            '2021OROZ01','2015TERR01','2017MART94','2023VELE05','2012PERE04','2024MEJI05',
            '2023LOPE96','2024LOPE19','2023PINH03','2019RODR66','2014BULU01','2018CRUZ17',
            '2023STRA33','2023GODO07','2023MAZU05','2018GONZ30','2023GOME49'
          ]
        },
        {
          name: 'FTO', groups: [123, 473, 474],
          competitors: [...new Set([
            '2023RODR80','2023SUAR08','2022QUIN17','2019GUTI14','2016CRUZ16','2016BARO02',
            '2017POPA01','2019BUIT01','2014ARGA01','2023MORR23','2016SUZU03','2022CUER01',
            '2024ESCA04','2015TERR01','2023VILL10','2017MART94','2024GOOS03','2024LOPE19',
            '2019DEVE02','2023PINH03','2019RODR66','2014BULU01','2018GONZ30'
          ])]
        },
      ]

      // TB pairs — both go to same group
      const tbPairs = [
        ['2023BELT07','2023CELI06'], ['2014AMAY01','2022SAND19'], ['2023SUAR08','2021SOTO01'],
        ['2022QUIN17','2023MORE50'], ['2019GUTI14','2024GUTI02'], ['2017POPA01','2017MUNO06'],
        ['2014ARGA01','2016GONZ52'], ['2024COLO04','2023MORR23'], ['2025FUEN05','2023MONT31'],
        ['2023FLOR03','2021OROZ01'], ['2017CUES02','2022PUEN01'], ['2017MART94','2017TOYS01'],
        ['2023VELE05','2022CUER01'], ['2018GONZ09','2017SANC20'], ['2024MEJI05','2024ORTI07'],
        ['2023LOPE96','2024LOPE19'], ['2023PINH03','2023VIDA12'], ['2019RODR66','2014BULU01'],
        ['2016CARD07','2019DEVE02'], ['2023STRA33','2023GODO07'],
      ]
      ...
      for (const ev of events) {
        const perGroup = Math.ceil(ev.competitors.length / 3)
        for (let i = 0; i < ev.competitors.length; i++) {
          const wcaId = ev.competitors[i]
          const person = afterPhase3.persons.find(p => p.wcaId === wcaId)
          if (!person) continue

          const defaultG = i < perGroup ? 0 : i < perGroup * 2 ? 1 : 2
          let bestG = defaultG
          const defInfo = actMap[ev.groups[defaultG]]
          if (defInfo && hasConflict(person, defInfo.start, defInfo.end)) {
            // Try to find a conflict-free group
            let found = false
            for (let g = 0; g < 3; g++) {
              if (g === defaultG) continue
              const altInfo = actMap[ev.groups[g]]
              if (altInfo && !hasConflict(person, altInfo.start, altInfo.end)) { bestG = g; found = true; break }
            }
            // If no conflict-free group, pick the one with least overlap
            if (!found) {
              let minOverlap = Infinity
              for (let g = 0; g < 3; g++) {
                const gInfo = actMap[ev.groups[g]]
                if (!gInfo) continue
                let overlap = 0
                for (const a of person.assignments) {
                  const ai = actMap[a.activityId]
                  if (!ai) continue
                  if (ai.start < gInfo.end && gInfo.start < ai.end) {
                    const os = ai.start > gInfo.start ? ai.start : gInfo.start
                    const oe = ai.end < gInfo.end ? ai.end : gInfo.end
                    overlap += new Date(oe+'Z').getTime() - new Date(os+'Z').getTime()
                  }
                }
                if (overlap < minOverlap) { minOverlap = overlap; bestG = g }
              }
            }
          }

          person.assignments.push({ activityId: ev.groups[bestG], assignmentCode: 'competitor', stationNumber: null })
          totalAssigned++
        }
      }

      // TB pairs
      const pairsPerGroup = Math.ceil(tbPairs.length / 3)
      for (let i = 0; i < tbPairs.length; i++) {
        const pair = tbPairs[i]
        const defaultG = i < pairsPerGroup ? 0 : i < pairsPerGroup * 2 ? 1 : 2
        let bestG = defaultG
        const defInfo = actMap[tbGroups[defaultG]]

        if (defInfo) {
          const p1 = afterPhase3.persons.find(p => p.wcaId === pair[0])
          const p2 = afterPhase3.persons.find(p => p.wcaId === pair[1])
          if ((p1 && hasConflict(p1, defInfo.start, defInfo.end)) || (p2 && hasConflict(p2, defInfo.start, defInfo.end))) {
            let found = false
            for (let g = 0; g < 3; g++) {
              if (g === defaultG) continue
              const altInfo = actMap[tbGroups[g]]
              const c1 = p1 ? hasConflict(p1, altInfo.start, altInfo.end) : false
              const c2 = p2 ? hasConflict(p2, altInfo.start, altInfo.end) : false
              if (!c1 && !c2) { bestG = g; found = true; break }
            }
            if (!found) {
              let minOverlap = Infinity
              for (let g = 0; g < 3; g++) {
                const gInfo = actMap[tbGroups[g]]
                if (!gInfo) continue
                let overlap = 0
        ...
        for (const wcaId of pair) {
          const person = afterPhase3.persons.find(p => p.wcaId === wcaId)
          if (!person) continue
          person.assignments.push({ activityId: tbGroups[bestG], assignmentCode: 'competitor', stationNumber: null })
          totalAssigned++
        }
      }

      console.log(`  Assigned ${totalAssigned} unofficial competitor slots`)

      // Second pass: reoptimize each person to the group with least overlap
      const allEvGroups = [...events.map(e => e.groups), tbGroups].flat()
      let improved = 0
      for (const p of afterPhase3.persons) {
        for (const ev of [...events, { name: 'TB', groups: tbGroups }]) {
          const assignIdx = p.assignments.findIndex(a => ev.groups.includes(a.activityId) && a.assignmentCode === 'competitor')
          if (assignIdx === -1) continue

          const currentId = p.assignments[assignIdx].activityId
          const currentInfo = actMap[currentId]
          if (!currentInfo) continue

          let currentOv = 0
          for (const a of p.assignments) {
            if (allEvGroups.includes(a.activityId)) continue
            const ai = actMap[a.activityId]
            if (!ai) continue
            if (ai.start < currentInfo.end && currentInfo.start < ai.end) {
              const os = ai.start > currentInfo.start ? ai.start : currentInfo.start
              const oe = ai.end < currentInfo.end ? ai.end : currentInfo.end
              currentOv += new Date(oe+'Z').getTime() - new Date(os+'Z').getTime()
            }
          }
          if (currentOv === 0) continue

          let bestId = currentId, bestOv = currentOv
          for (const gId of ev.groups) {
            const gInfo = actMap[gId]
            if (!gInfo) continue
            let ov = 0
            for (const a of p.assignments) {
              if (allEvGroups.includes(a.activityId)) continue
              ...
              if (ai.start < gInfo.end && gInfo.start < ai.end) {
                const os = ai.start > gInfo.start ? ai.start : gInfo.start
                const oe = ai.end < gInfo.end ? ai.end : gInfo.end
                ov += new Date(oe+'Z').getTime() - new Date(os+'Z').getTime()
              }
            }
          ...
          if (bestId !== currentId) {
            p.assignments[assignIdx].activityId = bestId
            improved++
          }
        }
      }
